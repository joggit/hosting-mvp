...
app.logger.info(f"✅ Site name validation passed: '{site_name}'")
            ...
            if domain_config:
                ...
                if full_domain:
                    conn = get_db()
                    cursor = conn.cursor()
                    cursor.execute("SELECT COUNT(*) FROM domains WHERE domain_name = ?", (full_domain,))
            ...
            if 'package.json' in project_files:
                try:
                    ...
                    if package_data.get('type') == 'module':
                        ...
                        fixes_applied.append("Removed 'type: module' for PM2 compatibility")
            ...
            if 'next.config.js' not in project_files and 'next.config.mjs' not in project_files:
...
module.exports = nextConfig;"""
                ...
                try:
                    ...
                    if 'build' in package_data.get('scripts', {}):
                        ...
                        if build_result.returncode == 0:
                            ...
                            app.logger.warning(f"Build had warnings: {build_result.stderr}")
                except:
                    pass
            ...
            try:
                ...
                if pm2_check.returncode == 0:
                    ...
                    app.logger.info("Starting as background process...")
                    subprocess.Popen(
                        ['npm', 'start'],
                        cwd=app_dir,
                        env={**os.environ, 'PORT': str(allocated_port)},
        ...
        proxy_set_header X-Forwarded-Proto $scheme;
            ...
            if full_domain:
                ...
                if deployment_type == "subdomain":
                    ...
                    response_data["domain"]["parent_domain"] = domain_config['parent_domain']
            else:
