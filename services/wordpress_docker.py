...
# ═══════════════════════════════════════════════════════════════════════════════
# ADD to services/wordpress_docker.py
# ═══════════════════════════════════════════════════════════════════════════════
#
# Add this function after the existing create_site() function.
# register_site() differs from create_site() in that:
#   - It takes credentials pre-generated by the client (.env)
#   - It pulls the Docker image from Docker Hub (dockster1/site_name:latest)
#   - It receives the docker-compose.prod.yml content from the client
#   - No theme file upload needed — image already contains wp-content
# ───────────────────────────────────────────────────────────────────────────────
...
def register_site(
    ...
    db_password: str,
    db_root_password: str,
    dockerhub_username: str = "dockster1",
    compose_content: str = None,
) -> dict:
    """
    Register a new WordPress site that is deployed via Docker Hub image.

    Unlike create_site() which uploads theme files, this function:
      1. Creates the site directory on the server
      2. Writes the docker-compose.prod.yml (passed from client or auto-generated)
      3. Writes a minimal .env for the production stack
      4. Pulls the image from Docker Hub
      5. Starts the containers
      6. Waits for MySQL to be ready
      7. Writes the nginx vhost
      8. Registers the site in the SQLite database

    The WordPress database is populated separately via the import endpoint
    (full_production_push.py --db-only --via-api).

    Args:
        site_name:          Site identifier — used as container prefix
        domain:             Production domain e.g. wpdevops001.smartwave.co.za
        port:               Production container port (unique per site)
        db_name:            MySQL database name
        db_user:            MySQL user
        db_password:        MySQL password
        db_root_password:   MySQL root password
        dockerhub_username: Docker Hub account (default: dockster1)
        compose_content:    docker-compose.prod.yml content. If None, auto-generated.

    Returns:
        {site_name, domain, port, url, admin_url}
    """
    logger.info("=" * 55)
    logger.info(f"  WordPress Register Site: {site_name}")
    logger.info(f"  Domain: {domain}  Port: {port}")
    logger.info("=" * 55)

    site_path = WORDPRESS_BASE_DIR / site_name
    site_path.mkdir(parents=True, exist_ok=True)
    (site_path / "wp-content" / "uploads").mkdir(parents=True, exist_ok=True)
    logger.info(f"  Site directory: {site_path}")

    image = f"{dockerhub_username}/{site_name}:latest"

    # ── Write docker-compose.prod.yml ─────────────────────────────────────────
    if compose_content:
        (site_path / "docker-compose.prod.yml").write_text(compose_content)
        logger.info("  Wrote docker-compose.prod.yml (from client)")
    else:
        # Auto-generate a minimal production compose
        auto_compose = f"""services:
  wordpress:
    image: {image}
    container_name: {site_name}-wordpress
    restart: always
...
networks:
  internal:
    driver: bridge
"""
        (site_path / "docker-compose.prod.yml").write_text(auto_compose)
        logger.info("  Wrote docker-compose.prod.yml (auto-generated)")

    # ── Write .env for production stack ───────────────────────────────────────
    (site_path / ".env").write_text(
        f"SITE_NAME={site_name}\n"
        f"DOMAIN={domain}\n"
        f"PROD_PORT={port}\n"
    ...
    _run(f"docker pull {image}", f"Pull {image}")

    # ── Start containers ──────────────────────────────────────────────────────
    _run(
        f"cd '{site_path}' && docker compose -f docker-compose.prod.yml up -d",
    ...
    _run(
        f"docker exec {site_name}-wordpress chown -R 33:33 /var/www/html/wp-content/uploads",
        "Fix uploads ownership (www-data UID 33)",
        check=False,
    )
    ...
    logger.info(f"  Image: {image}  Port: {port}")
    logger.info("=" * 55)
